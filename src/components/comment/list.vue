<template>
  <div class="comments">
    ...
    <div class="evaluate" :style="{overflow:'auto',height:data.length>0?winH+'px':''}" v-infinite-scroll="load"
         infinite-scroll-disabled="disabled" v-if="$route.name!='lessonDetail'">
      <div class="list">
        <div class="every" v-if="data&&data.length>0" v-for="(item,index) in data" :key="index">
          ...
          <div class="word">
            ...
            <div class="other">
              ...
              <ul>
                <li
                  @click="$router.push({path:'/lenCommentDetail',query:{id:$route.query.id,mainId:item.id,type:8}})">
                  <img src="@/assets/img/detail/eval.png" alt="">{{$route.query.type!='话题详情'?item.replyNum:item.score}}
                </li>
    ...
    <!--课程评论列表-->
    <div class="evaluate" v-if="$route.name=='lessonDetail'">
      ...
      <div class="more" @click="toMore()">{{data && data.length > 0 ? '查看更多留言>>' : '快去评论吧...'}}</div>
    </div>
  </div>
</template>
...
<script>
  ...
  import {getCommentAPI, saveLenComAPI,lenssonAgreeAPI} from '@/api/detail.js';
  export default {
    name: 'HelloWorld',
    props: ['resource'],
    data() {
    ...
    created() {
      ...
      this.winH = document.documentElement.clientHeight - (winW * 90 / 375) - (winW * 60 / 375);
    ...
    methods: {
      ...
      getCommentList() {
        this.loading = 0;
          getCommentAPI({
            ...
            pageSize: this.$route.name == 'lessonDetail' ? 20 : 10,
          }).then(res => {
            if (res.data.status) {
      ...
      toMore() {
        this.$router.push({path: '/lenssonComment', query: {id: this.$route.query.id, type: this.object[this.$route.name]}});
      },
