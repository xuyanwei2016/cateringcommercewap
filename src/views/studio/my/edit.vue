...
<script>
  ...
  import {getStudioInfoAPI,saveInfoAPI,uploadSchoolAPI} from '@/api/studio/my.js';
  ...
  export default {
    ...
    methods: {
      ...
      onSubmit(){
        ...
        //   /*title: 职称*/
        // };
        // let status = this._validator(this.detail)
        // if(status){
          this.detail.faceImg = this.imgId
          saveInfoAPI(this.detail).then(res=>{
            if(res.data.status){
              this.$valert.show('修改成功');
              this.$router.push('/personalStudio');
            }
          })
        // }
      },
