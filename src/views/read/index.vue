    ...
    <div class="cont readCont" v-if="detail.title" >
      ...
      <h2>
        ...
        <div><!--v-show="detail.isFree==0"-->
          <span v-if='detail.price'>￥<del>{{detail.price}}</del></span>
          <span v-if='detail.realPrice'>￥{{detail.realPrice}}</span>
        </div>
        <!-- <span  v-if='detail.source'>来源: {{detail.source}}&nbsp;&nbsp;&nbsp;&nbsp;{{detail.updateTime}}</span>
        <span style="padding: 0">
          <i v-if='detail.price'>￥</i><del v-if='detail.price'>{{detail.price}}</del>
          <i>￥</i>{{detail.realPrice}}</span> -->
      </h2>
      <div v-if="!((detail.readTextPower!=0&&!token)||(detail.readTextPower!=0&&token&&detail.isFree==0))||!detail.text">
        ...
        <!--<audio :src="'http://122.14.50.6:2018/2020/03/20/79496399f9d7491693484660830f3e7a.mp3'" controls>
        </audio>-->
      ...
      <!--1111
      <video src="http://122.14.50.6:2018/2020/03/17/39c66c5457984f9cbb643a094ae35cab.mp4"></video>-->
    </div>
...
<script>
  ...
  import {increaseLearningNumAPI,saveCollectionAPI,deleteCollectionAPI} from '@/api/xnew.js';
      ...
      getDetail(){
        ...
        getArticleDetailAPI(this.id).then(res=>{
          if(res.data.status){
            ...
            console.log(this.detail,'1111111222')
            ...
            if(this.detail.audio&&this.detail.audio!='无权限'){
              getImgLinkAPI(this.detail.audio).then(res=>{
                this.src='http://192.168.2.242:8501/'+res.data.data;
              })
            }
      ...
      let player = new ckplayer(this.videoObject);*/
        if(this.detail.video&&this.detail.video!='无权限') {
          getImgLinkAPI(this.detail.video).then(res => {
            ...
            this.videoObject.video = 'http://192.168.2.242:8501/2020/03/17/39c66c5457984f9cbb643a094ae35cab.mp4';
...
/*
      ...
      collectionFn(){
        ...
        if(!this.token){
          ...
          return}
        let API=this.detail.isCollection?deleteCollectionAPI:saveCollectionAPI;
        API({
          followId: this.id,
          followType: '2-2',//收藏对象类型 供查询用 2-1医著 2-2文章 2-3 课程（医道传承） 2-视频（医道传承） 2-5病例（医道传承） 医到传承-大师：51
          memberType:3
          }
        ).then(res => {
          if (res.data.status) {
...
<style scoped lang="less">
  .read {
    ...
    .cont {
      ...
      padding: .7rem 0.12rem 0.55rem 0.12rem;
      h1 {
        color: #333;
        font-size: 0.24rem;
