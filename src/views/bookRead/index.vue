<template>
  ...
  <div class="bookRead" :style="{ background: `rgba(${backColor[isBcolor]}`+(range/100)+')' }">
    ...
    <div class="foot" :style="{paddingBottom:!judgeBigScreen()?'0':'.15rem'}">
      <ul>
    ...
    <div class="Popup" v-if="isShow" @click="isShow=false">
      ...
      <div class="Catalog" @click.stop="" :style="{height:clientH+'px'}" v-if="isSCont==2">
        ...
        <div :class="{list:true,lock:detail.price&&item.pid.indexOf('txt001')<0?true:false}"  v-for="(itm,inx) in chapterList" :key="inx" @click="changDirectory(itm,inx)" v-if="chapterList.length>0">
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      console.log(222, document.body.clientHeight,this.clientH,this.winH)
      this.getDetail();
    },
    watch: {},
    computed:{
...
<style scoped lang="less">
  ...
  .bookRead {
    ...
    .content {
      padding: 1.1rem .145rem 0 .145rem;
      color: #333;
      font-size: .18rem;
      line-height: .28rem;
      text-indent: 2em;
