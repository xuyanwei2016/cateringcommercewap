  ...
  <div class="phoneVerification">
    ...
    <ul class="list">
      <li><span>{{$route.name == 'phoneVerification' ? '手机号' : '新手机号'}}</span>
        <input type="text" placeholder="输入手机号" v-model="phone" @change="phoneFn"  @focus="getFocus"></li>
      <li><span>验证码</span>
        <div class="code">
          <input type="text" placeholder="输入验证码" v-model="code" @focus="getFocus">
          <h1 @click="!isShow&&getCode()">{{isShow ? `重新获取(${time}s)` : '获取验证码'}}</h1>
        </div>
      </li>

    </ul>
    <div class="button active" @click="submit" v-if="$route.name != 'phoneVerification2'">
     提交
    </div>

    <div class="button active" @click="submitNew" v-if="$route.name == 'phoneVerification2'">
      验证新手机
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      phoneFn() {
        /*/^[0-9]*$/*/
        let str=this.phone.trim();

        if (this.phone.trim().length != 11||!(/^[0-9]*$/.test(str))) {
...
<style scoped lang="less">
  .phoneVerification {
    ...
    .active {
      ...
      color: #fff;
      font-weight: bold;
      box-shadow: 0.01rem .03rem .06rem 0rem rgba(186, 159, 116, 0.5);
